...
def main():
    ...
    # 严格过滤：只保留智能网联汽车相关
    icv_filtered = [item for item in unique if filter_icv(item["title"])]
    filtered_out = len(unique) - len(icv_filtered)
    if filtered_out > 0:
        print(f"  过滤非ICV: {filtered_out}条")

    # 分类
    articles = []
    unclassified = []
    for item in icv_filtered:
    ...
    # 保存未分类文章
    if unclassified:
        unclassified_path = os.path.join(BASE, "data", f"unclassified_{WINDOW}.json")
        ...
        print(f"  未分类: {len(unclassified)} 条 → {unclassified_path}")
